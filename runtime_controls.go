...
func (a *RuntimeAgent) LoopRuntimeOptions(_ context.Context, loopID uuid.UUID) (tui.LoopRuntimeOptions, error) {
	...
	model := handle.Model()
	options.Models = []tui.ModelOption{{ID: tui.ModelID(modelID(model)), Label: model.Name, Description: string(model.Provider)}}
	if model.Caps.Thinking {
		for _, effort := range []inference.Effort{inference.EffortNone, inference.EffortLow, inference.EffortMedium, inference.EffortHigh, inference.EffortMax} {
			label := string(effort)
			if label == "" {
				label = "Model default"
			}
...
func (a *RuntimeAgent) SetModel(ctx context.Context, loopID uuid.UUID, id tui.ModelID) error {
	...
	if modelID(model) != string(id) {
		return fmt.Errorf("coderig: model choice %q is stale or unknown", id)
	}
	return controller.Change(ctx, loop.ChangeModel(model))
}
...
func modelID(model inference.Model) string { return string(model.Provider) + "/" + model.Name }
