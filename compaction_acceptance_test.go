...
func openAcceptanceAgentWithContextPolicy(t *testing.T, client inference.Client, counter inference.ContextCounter) *sessionAdapter {
	t.Helper()
	model := testModel()
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			counterCapability := inference.CounterCapability{
				...
				TokenizerRev: "coderig-acceptance-exact-v1", Quality: inference.CountQualityExactLocal,
			}
