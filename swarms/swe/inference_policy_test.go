...
func TestValidateStandardModelContext(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{name: "kimi window resolves", model: chutesKimiK26()},
		{name: "phala window resolves", model: phalaGLM52()},
		{name: "lm studio unknown limit is rejected", model: lmStudioLocal("local-model"), wantErr: true},
		{name: "lm studio configured input cap resolves", model: func() inference.Model {
			value := lmStudioLocal("local-model")
			value.Limits = inference.ContextLimits{MaxInputTokens: 32_000}
			return value
		}()},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := validateStandardModelContext(tt.model)
			if (err != nil) != tt.wantErr {
				t.Fatalf("validateStandardModelContext() error = %v, wantErr %v", err, tt.wantErr)
			}
			if tt.wantErr {
				var target *StandardModelContextError
				if !errors.As(err, &target) {
					t.Fatalf("error = %T %v, want *StandardModelContextError", err, err)
				}
				if target.Model != tt.model.Key() {
					t.Errorf("Model = %+v, want %+v", target.Model, tt.model.Key())
				}
			}
		})
	}
