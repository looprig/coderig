...
func (e *acceptanceCompactionFixtureError) Error() string {
	if e.Cause == nil {
		return "swe test: invalid compaction fixture field " + e.Field
	}
	return "swe test: invalid compaction fixture field " + e.Field + ": " + e.Cause.Error()
}

func (e *acceptanceCompactionFixtureError) Unwrap() error { return e.Cause }

func openAcceptanceAgentWithClient(t *testing.T, client inference.Client) (*sessionAgent, *swarmStores) {
	t.Helper()
		...
		t.Run(tt.name, func(t *testing.T) {
			client := &fakeLLM{
				streamSteps: []fakeStreamStep{
					{chunks: []content.Chunk{&content.TextChunk{Text: "first reply"}}, result: &inference.StreamResult{Usage: &turnUsage}},
					{chunks: []content.Chunk{&content.TextChunk{Text: "second reply"}}, result: &inference.StreamResult{Usage: &secondUsage}},
				},
			...
			idle, ok := agent.sess.(interface{ WaitIdle(context.Context) error })
			if !ok {
...
func TestAcceptanceCompactionRejectsEveryXMLFailureDomain(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			client := &fakeLLM{
				streamSteps: []fakeStreamStep{{chunks: []content.Chunk{&content.TextChunk{Text: "reply"}}, result: &inference.StreamResult{Usage: &turnUsage}}},
...
func (c *acceptanceContextCounter) CountContext(_ context.Context, request inference.Request) (inference.ContextCount, error) {
	c.mu.Lock()
	defer c.mu.Unlock()
	if len(c.counts) == 0 {
		return inference.ContextCount{}, &acceptanceCompactionFixtureError{Field: "counter script"}
	}
	count := c.counts[0]
	if len(c.counts) > 1 {
		c.counts = c.counts[1:]
...
func (c *acceptanceContextCounter) CounterCapability() inference.CounterCapability {
	return c.capability
}

func openAcceptanceAgentWithContextPolicy(t *testing.T, client inference.Client, counter inference.ContextCounter) *sessionAgent {
	t.Helper()
	model := testModel()
	...
	policy := conversationContextPolicy{
		...
		capability: inference.InferenceCapability{
			Provider: inference.ProviderID(model.Provider), Transport: inference.InferenceTransportLocal, Retention: inference.RetentionNone,
		},
	...
	definitions, err := swarmDefinitionsWithContextPolicy(client, model, Config{}, policy)
	if err != nil {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			counterCapability := inference.CounterCapability{
				...
				TokenizerRev: "swe-acceptance-exact-v1", Quality: inference.CountQualityExactLocal,
			}
...
func (*acceptanceFinalizationError) Error() string {
	return "swe test: compaction terminal append failed"
}
