...
const greetingLead = "CodeRig is a software-engineering swarm. It plans a task and delegates to specialist agents."
...
func Greeting(cfg Config) string {
	...
	return buildGreeting(greetingRoster(), greetingSkills())
}

// greetingRoster returns every user-visible leaf in definition order. operator is listed once;
// the internal operator-primary primer is topology, not a second public agent identity. It is
// derived from leafBuiltins, the same source used by the Rig and Subagent catalog,
// so the greeting can never name an agent the Rig does not wire.
func greetingRoster() []LoopDisplay {
	builtins := leafBuiltins()
	out := make([]LoopDisplay, 0, len(builtins))
	for _, b := range builtins {
		out = append(out, LoopDisplay{Name: b.name, Description: b.description})
	}
	return out
}

// greetingSkills returns the de-duplicated, sorted set of embedded skill names wired
// across all leaves. This is the cheap, definition-sourced skill listing for the greeting. It
...
func buildGreeting(roster []LoopDisplay, skills []string) string {
	if len(roster) == 0 {
		return ""
	}
