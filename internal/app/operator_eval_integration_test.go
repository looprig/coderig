...
// operatorRunner adapts the live operator root-primer sessionAdapter to eval.Runner:
...
// projection from text_test.go — this test is in package coderig, so the unexported
...
type operatorRunner struct{ agent *sessionAdapter }
...
func newOperatorRig(ctx context.Context, client inference.Client, factory ModelFactory) (*sessionAdapter, error) {
...
func TestOperatorEvalIntegration(t *testing.T) {
	...
	// Build the shared client + key-bound ModelFactory the same way coderig.New does
