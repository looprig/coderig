...
// secret-free model.Model identity (the package default) verbatim: same provider, API format,
...
func TestModelFactoryYieldsSharedModel(t *testing.T) {
	...
	if got.Provider != defaultModel.Provider {
		t.Errorf("factory().Provider = %q, want %q", got.Provider, defaultModel.Provider)
	}
	if got.APIFormat != defaultModel.APIFormat {
		t.Errorf("factory().APIFormat = %q, want %q", got.APIFormat, defaultModel.APIFormat)
	}
	if got.BaseURL != defaultModel.BaseURL {
		t.Errorf("factory().BaseURL = %q, want %q", got.BaseURL, defaultModel.BaseURL)
	}
	if got.Name != defaultModel.Name {
		t.Errorf("factory().Name = %q, want %q", got.Name, defaultModel.Name)
	}
}
...
func TestBuildClientHappy(t *testing.T) {
	...
	if got := factory(); got.Name != defaultModel.Name {
		t.Errorf("factory().Name = %q, want the shared model %q", got.Name, defaultModel.Name)
	}
}
