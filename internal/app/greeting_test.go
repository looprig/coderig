...
// TestGreetingFromDefinitions pins the definition-sourced greeting: when on, it lists the
// real wired leaves; when off it is empty. It also asserts the skill source is the
// definitions' embedded skill names, so the greeting cannot drift from the wiring.
func TestGreetingFromDefinitions(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{name: "toggle on yields definition-derived greeting", enabled: true},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got == "" {
				t.Fatal("Greeting(on) = empty, want a greeting derived from definitions")
			}
			// Every wired leaf must be named.
			for _, b := range leafBuiltins() {
				if !strings.Contains(got, string(b.name)) {
					t.Errorf("greeting missing wired agent %q:\n%s", b.name, got)
				}
			}
			// The operator's embedded skill is listed from the fixed definitions.
