...
// TestIdentityNonEmpty proves the shared CodeRig identity prompt is present: the
...
func TestIdentityContainsCrossCuttingBits(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "names the CodeRig product", want: "CodeRig"},
...
func TestIdentityIsWellFormedXML(t *testing.T) {
	...
	if probe.Product != "CodeRig" {
		t.Errorf("Identity product attr = %q, want %q", probe.Product, "CodeRig")
	}
}
